...
After performing MRP, we predicted that Joe Biden will wi
...
## Weaknesses, Limitations, and Possible Next Steps {#sec-weaknesses-limitations}
...
@fig-stanareyouokay-2 shows the trace plot for our intercept. We can see that the lines bounce, but remain horizontal and have a lot of overlap with each other [@tswd]. @fig-tracesex, @fig-traceage, @fig-tracerace, @fig-traceeducation, and @fig-traceurban show the trace plots for our sex, age_bracket, races, education_level, and urban variables. Each of these trace plots looks similar to @fig-stanareyouokay-2, as the lines bounce, remain horizontal, and overlap a lot. Our trace plots suggest that the distribution behaves as expected, and we do not need to re-run our model with fewer predictors or different priors [@tswd].
...
plot(presidential_vote_model, "trace","(Intercept)") 
...
In @fig-modelplot, we show the 90% credibility intervals for the predictors of `vote_biden`. We see that the 90% credibility interval for when urban=urban is greater than 0. This means that a respondent who lives in an urban area is more likely to support Joe Biden. We also see that the 90% credibility interval for sex=male is below zero. This means that based on our survey data and model, male respondents are more likely to support Donald Trump.
